...
1. **ubi2-oc 比预想更"前沿"**：= openwrt master @2026-08-16（`20d94d5a2b27`，内核 6.18.44）+ 恰好 20 条定制 commit，**0 条落后**；其 `ubi2-oc-auto` 通道每 2 小时自动 rebase 上游并 force-push。
...
| 项 | 事实 |
|---|---|
| head | `ed7cbc80` "switch to performance governor and overclock +200mhz"（author 2026-02-14，整枝 rebase 于 08-16） |
| base | openwrt master `20d94d5a2b27`（2026-08-16T10:33Z），内核 6.18.44 |
| 相对 upstream | ahead 20 / behind 0（merge_base = openwrt main tip） |
| 自身分支 | `ubi2` = ubi2-oc 去顶 commit（stock 档）；`ubi2-oc-auto` 为自动 rebase 通道；旧 OC sha `80096373b5` 已在 rebase 中重写不可达 |
...
| 补丁形态 | 独立 patches/ + 应用脚本 + 元数据 | 20 条 commit 直压分支 | 我们的模型胜（可追溯、可剥离） |
...
| | fanboy 模型 | 我们的模型 |
|---|---|---|
| 改动载体 | 20 条 commit 直压分支 | patches/ 分层 + 应用脚本 |
...
3. **OC 三件套**从 `ed7cbc80` 提取为独立补丁组（opp 表 + PLL 公式 + governor），做成：stock（不带）/ oc-1.3g（保守，opp 上限 1300 或运行时 maxfreq 限制）/ oc-1.4g（激进）三形态，采用"同源去顶 + tag 区分"；
