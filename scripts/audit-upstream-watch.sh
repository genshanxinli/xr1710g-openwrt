...
REMOTE_TIPS=(
  ...
  "3d4ff76d3|https://github.com/Gilly1970/Gemtek-W1700K-6.18.git|HEAD|Gilly openwrt-patches/（F90/F101 对账源；a57615652=Bump Kernel-6.18.51 and rebase + 新补丁 048，048 已吸收为 mt76-0022/F153；F171/P17 复核 a57615652→3d4ff76d3：快进 1 提交「Bump Kernel-6.18.52 and rebase」，960/961/982 三个 patch 从 openwrt-patches/ **移除**（随 6.18.52 上游化），本仓 9045 内层 960/961、9042 内层 982 因 pin 仍 6.18.44 不受影响（内核 bump ≥6.18.52 时须删这三份内层）⇒ 无 Gilly 来料重取；**F176/P25 观察（未完整对账）**：tip 又前进 `3d4ff76d3→38806db2d`（「Update Kernel-6.18.52 SSH and rebase」，+2 文件 `openwrt-patches/985-PCI-mediatek-gen3-request-port-IRQ-for-affinity-control.patch`（端口 IRQ 取代 chained handler ⇒ 可调 smp_affinity；**pin/上游 master 仍是 chained+NO_AFFINITY**，本仓不携带该文件）与 `netifd-hotplug-20-packet-steering`（Wi-Fi 接口 `add` 后重跑 packet_steering）；另 `Openwrt_Gemtek_w1700k.sh` ±1 行 + 一个 itb 产物；目录 81 份新增 2 删 0；同目录 `033-…release-hwrro-sessions-on-teardown` 经逐 op 比对 = 本仓 07 的逐行等同体、`037-…fix-hwrro-delete-event-struct-size` 的 4 字节布局修正 pin 已有 ⇒ **均非新料**。**锚点仍留 `3d4ff76d3`，待 P26 完整逐料对账后再重锚**）"
