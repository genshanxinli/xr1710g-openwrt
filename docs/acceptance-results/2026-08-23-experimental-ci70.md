...
- 结论：**远程可验项大部分通过；发现并修复 1 个 LED sysfs 回归；F66 rdinit 修复被 HTTP U-Boot 默认 env 覆盖，仍为假警告；F69/F71/C2/C3/C4 等物理项待补测。**
...
- [~] B5 NPU offload：`luci.airoha_npu getStatus` npu_loaded=true，offload_bound/offload_total>0；PPE entries 有 LAN↔WAN IPv4/IPv6 条目。IPv6 `[HW_OFFLOAD]`/PPE BND v6 专项未做。
...
- [~] E2 版本元数据：`DISTRIB_DESCRIPTION='OpenWrt SNAPSHOT r0-3d1645e'`；未含档位标识（experimental），同 2026-08-21 遗留项。
...
3. **device-hw-probe B2.1 VEND1 寄存器仍未打通**：phytool 2.x 对 lan1/lan2 读 `0x103/0x104` 返回 `phy_read (-95)`；需换 MDIO 访问方式或确认 phytool C45 用法。
4. **DISTRIB_DESCRIPTION 缺少档位标识**：与 2026-08-21 记录同一遗留项。
