...
- [x] **DNS 基建前置（Phase 0，2026-09-08 定稿执行，F91）**：mosdns v5.3.4 stock 档供给（`feeds.custom.conf` 锁 `df6d67b`，P3 前置依赖）+ dnsmasq 归位 127.0.0.1#5353（`files/etc/uci-defaults/97`）+ `files/etc/mosdns/config.yaml`（国内 DoH 防污染 / geosite 分流 / lazy_cache / 国外降级）+ fake-ip 空规则预置（198.18.0.0/16）+ 防火墙（`96`）——科学上网（nikki/OpenClash）上线时按 Phase 1 二选一接入（mosdns fakeip 插件激活，或代理以 127.0.0.1:5353 为上游，杜绝泄漏）；DNS 验收=`docs/ACCEPTANCE.md` B7-B10（实机项按用户口径延后）
