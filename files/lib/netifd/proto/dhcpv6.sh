...
proto_dhcpv6_setup() {
	local config="$1"
	local iface="$2"
	# XR1710G (F117): 显式 dhcpv6 客户端去重 —— 同一 L3 设备上已有另一个 netifd 接口的
	# odhcp6c 时拒绝启动（否则 ubus 对象 odhcp6c.<dev> 注册 EEXIST 导致反复重启）。
