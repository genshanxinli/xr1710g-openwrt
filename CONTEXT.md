...
> 术语只记"是什么/边界在哪"，不含实现细节。决策见 `docs/adr/0003`（架构）、`0005`（DNS 裁决）、`0006`（方案二供给链）。

- **offload 不变量** — 数据面的三条不可触碰项：chain `forward` 的 `flow add @ft`、flowtable 设备表、`flow_offloading*` 开关注。本仓库的代理/组网代码**任何路径**都不得写它们（静态门禁 I1/I3）。它是"开代理不影响正常网络硬件加速"这条北极星的落点。
- **内核态分流** — 分流**判定**发生在 nft / 策略路由层，而不是用户态。只有被内核判定为"要代理"的流才离开快路径进入用户态；其余流完全不知道代理的存在。与之相对的是"用户态分流"（抓取全部流量后在用户态按域名/规则决定去向），后者会让被代理设备的全部流量失去硬件卸载。
- **CN 放行集合** — 内核 nft 里的 `china_ip4` / `china_ip6` interval 集合，代表"国内目的地址"。名单客户端访问该集合内的目的地址时被内核**提前 accept**，走原转发路径 ⇒ 保住硬件卸载。来源三段：随固件的只读快照 + 运行期更新（落 `/tmp`）+ `dnsmasq --nftset` 注入的域名级补充。
- **名单客户端（proxy_hosts）** — 被管理员显式列入 `global.proxy_hosts` 的源地址。**只有**它们会被捕获进用户态。非名单客户端与本组件不存在时行为完全一致——这是可验收的（`ppe/bind` 计数与 iperf3 对照）。
- **假 IP 作用域（fake-ip scope）** — fake-ip 应答**允许投递给谁**。本仓库**只允许**投递给名单客户端；普通客户端恒得真实 IP。若普通客户端拿到 `198.18.x`，而内核的放行/抓取规则只对名单生效，它就会既出不去也没人代理 ⇒ **不是慢，是断**。这是"作用域"而非"性能"问题。
- **fail-open** — 代理核心或 DNS 不健康时的失败方向：**撤掉抓取与劫持规则，让名单客户端退回直连**，而不是断网。与之相对的 fail-closed 会为了"保持代理"而牺牲连通性；本项目选 fail-open（与"失败不影响正常网络"一致）。代价是故障期名单客户端不被代理，必须在状态页明示。
- **恢复页重建布局** — HTTP U-Boot 恢复页在上传固件时按镜像大小**重建并缩放 `fit` 卷**的机制（同时 `rootfs_data` 吃掉剩余 PEB）。副作用 = 移除除 `ubootenv`/`ubootenv2`/`factory` 外的所有卷 ⇒ **overlay 与全部配置被清空**。因此"扩容"与"清配置"是**同一个动作**，不是两个。
- **DNS 前置层** — 装在客户端与"真正决定解析结果的组件"之间、参与 DNS 解析的中间件（AdGuardHome、mosdns、SmartDNS 等）。它的能力边界由**位置**决定：当 fake-ip 由后面的代理核心产生时，前置层**看不到**被代理域名的查询（那些查询在代理核心内部就被拦截了）⇒ 它既拦不住那部分广告，也管不到那部分分流。
- **连接级过滤 vs DNS 级过滤** — 两者作用在**不同层**：DNS 级过滤只能拦"真的走到了这个解析器的查询"，对 fake-ip 域名无效；连接级过滤（在代理核心的路由规则里 `reject`）作用在连接建立阶段，对 fake-ip 域名**同样生效**，且不新增进程。要拦被代理域名上的广告，只有连接级是对的层。
